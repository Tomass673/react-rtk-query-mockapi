...
import {useGetElectronicsQuery} from "../store/product/product.api";
...
const Electronics: FC = () => {
    ...
    const { data, isLoading, error } = useGetElectronicsQuery({limit: 5, page: page, sortBy: sortBy, order: order})
    if (error) {
        if ('status' in error) {
